...
module.exports = {
    ...
    execute: async function (message, args, prefix) {
        ...
        if (row.exchangeId === 0) return message.reply('Вы не участвуете в АДМ.');
        ...
        else if (!args.length || args[0] !== 'получатель' && args[0] !== 'даритель')
            return message.reply(`Нужно указать кому сообщение!\n \`${prefix}message <даритель/получатель> <сообщение>\`.\n\n` +
                `получатель – человек, которому вы дарите подаров (<@${row.partnerId}>).\nдаритель – человек, дарящий подарок вам (Дед Мороз).`);
            ...
            const gifterEmbed = new Discord.MessageEmbed()
                .setTitle(`__Вы получили сообщение от вашего подопечного ${message.author.tag}!__`)
                .setDescription('\n' + args.slice(1).join(' '))
                .setColor(config.embeds_color)
