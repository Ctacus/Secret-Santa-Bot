...
module.exports = {
    name: 'renotify',
    aliases: ['remind'],
    description: 'Напоминает участникам, кому они дарят подраки!',
    hasArgs: true,
    ...
    execute: async function (message, args, prefix) {
        ...
        if (!args.length || ['all', 'missed'].indexOf(args[0].toLowerCase()) < 0)
            return message.reply('Укажите параметер `missed`, чтобы уведомить тех, кто пропустил первое сообщение, либо `all`, чтобы заново напомнить всем об ответственности.');
